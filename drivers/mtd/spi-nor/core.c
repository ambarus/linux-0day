...
/**
 * spi_nor_dtr_read() - read data from flash memory by avoiding odd lengths and
 * addresses.
 * @nor:        pointer to 'struct spi_nor'
 * @from:       offset to read from
 * @len:        number of bytes to read
 * @buf:        pointer to dst buffer
 *
 * On DTR capable flashes reads cannot start or end at an odd address in
 * DTR mode. Extra bytes need to be read at the start or end to make sure both
 * the start address and length remain even.
 *
 * Return: number of bytes read successfully, -errno otherwise.
 */
static int spi_nor_dtr_read(struct spi_nor *nor, loff_t from, size_t len,
			    u8 *buf)
{
	u8 *tmp_buf;
	size_t tmp_len;
			...
			size_t *retlen, u_char *buf)
{
	...
	while (len) {
		...
		if (nor->read_proto == SNOR_PROTO_8_8_8_DTR)
			ret = spi_nor_dtr_read(nor, addr, len, buf);
		else
...
/**
 * spi_nor_dtr_write() - write data to flash memory by avoiding odd lengths and
 * addresses.
 * @nor:        pointer to 'struct spi_nor'
 * @to:         offset to write to
 * @len:        number of bytes to write
 * @buf:        pointer to src buffer
 *
 * On DTR capable flashes the writes cannot start or end at an odd address in
 * DTR mode. Extra 0xff bytes need to be appended or prepended to make sure the
 * start address and end address are even. 0xff is used because on NOR flashes
 * a program operation can only flip bits from 1 to 0, not the other way round.
 * 0 to 1 flip needs to happen via erases.
 *
 * Return: number of bytes written successfully, -errno otherwise.
 */
static int spi_nor_dtr_write(struct spi_nor *nor, loff_t to, size_t len,
			     const u8 *buf)
{
	...
	size_t *retlen, const u_char *buf)
{
	...
	for (i = 0; i < len; ) {
		...
		if (nor->write_proto == SNOR_PROTO_8_8_8_DTR)
			ret = spi_nor_dtr_write(nor, addr, page_remain,
						buf + i);
		else
