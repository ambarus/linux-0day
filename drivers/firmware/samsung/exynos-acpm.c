...
static void acpm_prepare_xfer(struct acpm_chan *achan, struct acpm_xfer *xfer)
{
	...
	memset(rx_data->cmd, 0, sizeof(*(rx_data->cmd)) * achan->mlen);
	if (xfer->rx.cmd)
 ...
 * configuration flags like xfer->flags.
 *
...
static int acpm_achan_alloc_cmds(struct acpm_chan *achan)
{
	...
	size = sizeof(*(achan->rx_data[0].cmd));

	for (i = 0; i < ACPM_SEQNUM_MAX; i++) {
		rx_data = &achan->rx_data[i];
		rx_data->cmd = devm_kcalloc(dev,
					    DIV_ROUND_UP_ULL(achan->mlen, size),
					    size, GFP_KERNEL);
		if (!rx_data->cmd)
			return -ENOMEM;
	}
