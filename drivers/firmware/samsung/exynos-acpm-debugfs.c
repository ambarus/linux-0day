				 ...
				 const union acpm_log_entry *log_entry)
{
	...
	if (log_entry->is_raw) {
		...
		/* report time in ns */
		time = (systicks * ACPM_APM_SYSTICK_PERIOD_PS) / 1000;
...
/**
 * logb_gprio_level_get() - get ACPM Log Buffer Group Priority logging level.
 ...
 * supporting a maximum of 16 Plugin IDs.
 * 0xf is Log Error level, and 0x0 is Log Debug level.
 */
...
/**
 * logb_gprio_level_set() - set ACPM Log Buffer Group Priority logging level.
 * @data:	pointer to the driver data.
