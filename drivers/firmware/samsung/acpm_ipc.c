 ...
 * @mbox_request:	pointer to the mailbox request.
 * @callback:	pointer to a callback function to be invoked upon
 *		completion of this request. This has the form:
 *		(callback)(struct exynos_acpm_work_data *work_data, int status)
 *		where:
 *		@work_data:	data structure representing the work.
 *		@status:	status returned from the mailbox controller.
...
static int exynos_acpm_set_rx(struct mbox_chan *mbox_chan,
			      struct mbox_request *req)
{
...
static void exynox_mbox_debug_dump(struct mbox_chan *mbox_chan)
{
	struct exynos_acpm *exynos_acpm = dev_get_drvdata(mbox_chan->mbox->dev);
	struct exynos_acpm_chan *chan = mbox_chan->con_priv;
	struct device *dev = mbox_chan->mbox->dev;

	dev_dbg(dev, "I:0x%x RX r:%u f:%u TX r:%u f:%u\n",
	       readl_relaxed(exynos_acpm->regs + EXYNOS_ACPM_INTSR0),
	       readl_relaxed(chan->rx.rear),
	       readl_relaxed(chan->rx.front),
	       readl_relaxed(chan->tx.rear),
	       readl_relaxed(chan->tx.front));
	dev_dbg(dev, "SR0:0x%x MR0:0x%X MSR0:0x%X,"
		" SR1:0x%x MR1:0x%X MSR1:0x%X\n",
		readl_relaxed(exynos_acpm->regs + EXYNOS_ACPM_INTSR0),
		readl_relaxed(exynos_acpm->regs + EXYNOS_ACPM_INTMR0),
		readl_relaxed(exynos_acpm->regs + EXYNOS_ACPM_INTMSR0),
		readl_relaxed(exynos_acpm->regs + EXYNOS_ACPM_INTSR1),
		readl_relaxed(exynos_acpm->regs + EXYNOS_ACPM_INTMR1),
		readl_relaxed(exynos_acpm->regs + EXYNOS_ACPM_INTMSR1));
}
					  ...
					  struct mbox_request *req)
{
	...
	do {
		...
		ret = exynos_acpm_set_rx(mbox_chan, req);
	...
	dev_err(dev, "%s Timeout! ch:%u s:%u bitmap:%lx, cnt_20us = %d\n",
		__func__, chan->id, seqnum, chan->bitmap_seqnum[0], cnt_20us);
	exynox_mbox_debug_dump(mbox_chan);
...
static int exynos_acpm_chans_init(struct exynos_acpm *exynos_acpm)
{
	...
	for (i = 0; i < exynos_acpm->num_chans; i++) {
		...
		dev_vdbg(dev, "%s ID = %d poll = %d, mlen = %d, qlen = %d\n",
			 __func__, chan->id, chan->polling, chan->mlen,
			chan->qlen);
	}
