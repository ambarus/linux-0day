...
#define ACPM_TMU_EMUL_TEMP	0x15
#define ACPM_TMU_HYSTERESIS	0x16
#define ACPM_TMU_REG_READ	0xfe
#define ACPM_TMU_REG_WRITE	0xff

#define ACPM_TMU_REG_MSB	GENMASK(15, 8)
#define ACPM_TMU_REG_LSB	GENMASK(7, 0)

#define ACPM_TMU_DATA_31_24	GENMASK(31, 24)
#define ACPM_TMU_DATA_23_16	GENMASK(23, 16)
#define ACPM_TMU_DATA_15_8	GENMASK(15, 8)
#define ACPM_TMU_DATA_7_0	GENMASK(7, 0)
...
int acpm_tmu_set_emul_temp(const struct acpm_handle *handle,
			   unsigned int acpm_chan_id, u8 tz, u8 temperature)
{
	union acpm_tmu_msg msg = {0};
	struct acpm_xfer xfer;

	msg.tx.type = ACPM_TMU_EMUL_TEMP;
	msg.tx.tzid = tz;
	msg.tx.data[0] = temperature;
...
int acpm_tmu_read_reg(const struct acpm_handle *handle,
		      unsigned int acpm_chan_id, u8 tmu_id, u16 offset, u32 *val)
{
	union acpm_tmu_msg msg = {0};
	struct acpm_xfer xfer;
	int ret;

	msg.tx.type = ACPM_TMU_REG_READ;
	msg.tx.rsvd0 = tmu_id;
	msg.tx.data[0] = FIELD_GET(ACPM_TMU_REG_LSB, offset);
	msg.tx.data[1] = FIELD_GET(ACPM_TMU_REG_MSB, offset);
	...
	*val = FIELD_PREP(ACPM_TMU_DATA_31_24, msg.rx.data[3]) |
	       FIELD_PREP(ACPM_TMU_DATA_23_16, msg.rx.data[2]) |
	       FIELD_PREP(ACPM_TMU_DATA_15_8, msg.rx.data[1]) |
	       FIELD_PREP(ACPM_TMU_DATA_7_0, msg.rx.data[0]);

	return 0;
}

int acpm_tmu_write_reg(const struct acpm_handle *handle,
		       unsigned int acpm_chan_id, u8 tmu_id, u16 offset, u32 val)
{
	union acpm_tmu_msg msg = {0};
	struct acpm_xfer xfer;

	msg.tx.type = ACPM_TMU_REG_WRITE;
	msg.tx.rsvd0 = tmu_id;
	msg.tx.data[0] = FIELD_GET(ACPM_TMU_REG_LSB, offset);
	msg.tx.data[1] = FIELD_GET(ACPM_TMU_REG_MSB, offset);
	msg.tx.data[2] = FIELD_GET(ACPM_TMU_DATA_7_0, val);
	msg.tx.data[3] = FIELD_GET(ACPM_TMU_DATA_15_8, val);
	msg.tx.data[4] = FIELD_GET(ACPM_TMU_DATA_23_16, val);
	msg.tx.data[5] = FIELD_GET(ACPM_TMU_DATA_31_24, val);
